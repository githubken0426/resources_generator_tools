...
public class Gen22AddJSP implements GenTask {

	@Override
	public String generate(Param p) throws Exception {
		// 根据 template\jsp\add.jsp中的内容，生成add.jsp文件
		String src = p.base_dir + SourceGeneratorConst.TEMPLATE_SOURCE_JSP_PATH +"add.jsp";
		String tar = p.generator_path + SourceGeneratorConst.TARGET_JSP_PATH + p.base_name;

		FileUtil.createDir(tar);
		tar = tar + File.separator+"add.jsp";
		...
		// <tr>
		// <td width="200px" align="right" nowrap="nowrap" bgcolor="#f1f1f1"
		// height="40px">orgid：</td>
		// <td>
		// <input type="text" id="orgid" name="config.orgid" tabindex="1"
		...
		int idx = 1;
		for (Field f : p.list) {
			if (StringUtil.isEmpty(f.getInsertflag())) {
				continue;
			}
			listData = listData + "\r\n\t\t\t\t\t\t" + "<tr>";
			listData = listData
			...
			listData = listData + "\r\n\t\t\t\t\t\t\t"
					...
					+ "\" name=\"entity." + f.getCamelName() + "\" tabindex=\""
					+ idx + "\" style=\"width:400px;margin-left:30px;\"/>";
			listData = listData + "\r\n\t\t\t\t\t\t\t" + "<span id=\""
					+ f.getCamelName()
